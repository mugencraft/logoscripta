...
describe("RepositorySystemListService", () => {
  ...
  describe("findOrCreateSystemList", () => {
    ...
    it.skip("should create a new list when not found", async () => {
      const { sut, mocks } = createTestSetup({
  ...
  describe("getItems", () => {
    it("should return items from the specified list", async () => {
      ...
      expect(mocks.queries.findItemsByListWithRelations).toHaveBeenCalledWith(
        sampleSystemList.id,
      );
    });
