...
interface MockRepositoryListQueriesOptions {
	...
	responses?: {
		...
		/** Map of listId to response for findItemsWithRepository method */
		findItemsWithRepository?: Record<string, RepositoryListItem[]>;
		/** Map of "listId-metadataType" to response for findItemsByMetadataType method */
		findItemsByMetadataType?: Record<string, RepositoryListItem[]>;
	...
	errorScenarios?: {
		...
		/** "listId-metadataType" combinations that should trigger findItemsByMetadataType errors */
		findItemsByMetadataTypeErrors?: string[];
		/** List ids that should trigger findItemsWithRepository errors */
		findItemsWithRepositoryErrors?: number[];
...
export function createMockRepositoryListQueriesPort(
	...
	return {
		...
		findItemsByMetadataType: vi
			.fn()
			.mockImplementation(async (listId: number, metadataType: string) => {
				const key = `${listId}-${metadataType}`;

				// Check for error scenarios
				if (
					options.errorScenarios?.findItemsByMetadataTypeErrors?.includes(key)
				) {
					throw new Error(`Failed to find items by metadata type: ${key}`);
				}

				// Check for custom responses
				if (
					options.responses?.findItemsByMetadataType &&
					key in options.responses.findItemsByMetadataType
				) {
					return options.responses.findItemsByMetadataType[key];
				}
		...
		findItemsWithRepository: vi
			.fn()
			.mockImplementation(async (listId: number) => {
				// Check for error scenarios
				if (
					options.errorScenarios?.findItemsWithRepositoryErrors?.includes(
						listId,
					)
				) {
					throw new Error(
				...
				// Check for custom responses
				if (
					options.responses?.findItemsWithRepository &&
					listId.toString() in options.responses.findItemsWithRepository
				) {
					return options.responses.findItemsWithRepository[listId.toString()];
				}
