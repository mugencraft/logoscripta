...
export class ChangeDetector<T extends object> {
	...
	private hasFullChange(oldEntity: T, newEntity: T): boolean {
		return this.config.trackedFields.some((field) => {
			const oldValue = get(oldEntity, field);
			const newValue = get(newEntity, field);
	...
	private getEntityId(entity: T): string {
		const id = get(entity, this.config.idField);
		if (id === undefined) {
	...
	/**
	 * Validates a field path string to ensure it is safe for lodash.get().
