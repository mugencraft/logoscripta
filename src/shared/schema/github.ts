...
export const repositories = sqliteTable("repositories", {
  ...
  updatedAt: integer("updated_at", { mode: "timestamp" })
    .notNull()
    .$type<Date>(),
  pushedAt: integer("pushed_at", { mode: "timestamp" }).notNull().$type<Date>(),
...
export const repositoryLists = sqliteTable(
  "repository_lists",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    name: text("name").notNull(),
...
export const repositoryListItems = sqliteTable(
  "repository_list_items",
  {
    ...
    metadata: text("metadata", { mode: "json" })
      .$type<RepositoryListItemMetadata>()
      .notNull(),
  },
  (table) => [
    primaryKey({ columns: [table.listId, table.fullName] }),
    check("full_name_check", sql`LENGTH(full_name) >= 3`),
  ],
);
