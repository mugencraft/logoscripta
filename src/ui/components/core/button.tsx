...
const buttonVariants = cva(
  // CUSTOM:  remove disabled:pointer-events-none, this will enable cursor-not-allowed but still react to hover and other events
...
function Button({
  ...
  size,
  asChild = false,
  disableCursor = false, // CUSTOM
  ...props
}: React.ComponentProps<"button"> &
  VariantProps<typeof buttonVariants> & {
    asChild?: boolean;
    disableCursor?: boolean; // CUSTOM: Better control for cursor-pointer
  }) {
  const Comp = asChild ? Slot : "button";

  // CUSTOM
  const cursorClass = disableCursor
    ? ""
    : props.disabled
      ? "cursor-not-allowed"
      : "cursor-pointer";
