    ...
    return data.map((row) => {
      ...
      for (const column of visibleColumns) {
        ...
        if ("accessorFn" in column.columnDef) {
          try {
            filteredRow[colId] = column.columnDef.accessorFn(row, 0);
          } catch {
        ...
        // Handle columns with accessorKey
          ...
          filteredRow[colId] = row[column.columnDef.accessorKey as keyof TData];
        }
        // Direct property access if no accessor is defined
        else if (row && typeof row === "object" && Object.hasOwn(row, colId)) {
          filteredRow[colId] = row[colId as keyof TData];
        }
      }
